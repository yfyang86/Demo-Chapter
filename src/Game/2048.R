...
clockRotateMat <- function(x) t(x)[,ncol(x):1]
# Matrix Rotate: inverse-clockwise
anticolockRotateMat <- function(x) apply(x,1,rev)
# Matrix Rotate: upside-down
upsidedownMat <- function(x) x[nrow(x):1,]
# Delete and merge 0's in a vector
del0vec <- function(x) {y=rep(0,length(x));n = sum(x!=0);if(n>0){y[1:n]=x[x!=0]}else{y=x};y}
# Delete and merge 0's in a matrix by column
...
#[c(1, 5, 382, 410, 413, 503, 552, 506, 555,556,641,645)]

Draw2048 <-function(stormatrix2,...){
	n=dim(stormatrix2)[1]
	plot(c(0, n+1), c(0, n+1), type = "n", xlab="", ylab="",
     main = "2048'");
    for (i in 1:n){
	  for (j in 1:n){
		 y = n - i
